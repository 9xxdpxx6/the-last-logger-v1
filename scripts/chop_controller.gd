...
## ШУМ удара (импульс): множитель к силе удара. Громкий руб слышно далеко (для скрытности). Плавное
## нарастание/затухание этого импульса делает сам игрок (огибающая в player._process).
@export var hit_noise: float = 1.0
...
func _on_axe_impact() -> void:
	...
	# Шум удара — по силе удара в этот момент (плавную звуковую волну строит огибающая игрока).
	_player.add_noise(power * hit_noise)

	# Направление РЕЖУЩЕЙ КРОМКИ в момент удара — ось Y топора. Голова (бокс 0.06×0.16×0.22) тонкая по X
	# (это ТОЛЩИНА лезвия), а кромка тянется вдоль Y; брать basis.x было ошибкой — зарубка выходила
	# ПОПЕРЁК лезвия. По кромке ориентируем прорезь зарубки (диагональный руб даёт диагональную зарубку).
	var edge := _axe.global_transform.basis.y
